...
All counters are 32-bit and free-running. The cycle counter wraps
every 2^32 cycles — minutes at any plausible core clock; benchmarks
take seconds, so software gets reliable deltas by reading once before
the measured region and once after, then subtracting (modular
...
MOV   R2, R1
ANDI  R2, #0x0F               ; ALU immediates are 2-operand destructive
CMPI  R2, #1
BNE   unsupported_isa
MOV   R2, R1
ANDI  R2, #0x10               ; FPU bit
BNE   has_fpu
```
...
and benchmarks. Software must take the clock rate from this register
rather than a baked-in constant — the frequency varies across boards,
core generations, and fmax revisions.
...
`SYSDEV_CPU` cycles / insns counters.  At one event per cycle a
counter exhausts its 32-bit range in minutes, comfortable for
benchmark-scoped deltas.
...
SUB   R8, R4                  ; R8 = after - before (delta)
...
down to 1 MHz (the divisor is a board constant set alongside the PLL
configuration). On a discrete build, it could be a standalone 1 MHz
crystal.
