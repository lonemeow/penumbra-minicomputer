...
| `GISel/PenumbraInstructionSelector.cpp` | Manual select(): ALU ops, G_CONSTANT (LLI/LLIS/LUI), G_GLOBAL_VALUE (LLI+LUI with lo16/hi16 target flags), G_LOAD/G_STORE (LDB/LDH/LDW and STB/STH/STW by memop size, frame-index folding), G_PTR_ADD (→ADD), G_PTRMASK (→AND), G_INTTOPTR/G_PTRTOINT/G_FREEZE (→COPY), G_FRAME_INDEX (fold into memop or LEAfi for escaped addresses), G_ICMP+G_BRCOND fold (CMP+Bcc), G_BR, G_PHI, G_SELECT, G_ZEXT/G_SEXT, COPY constraint. LLI+LUI pairs use SSA-correct intermediate vregs. |
