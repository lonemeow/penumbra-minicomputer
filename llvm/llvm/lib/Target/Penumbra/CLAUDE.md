...
| `PenumbraInstrInfo.td` | All 4 instruction formats (R/L/M/B) with bit-accurate encoding. Tied-operand constraints for 2-address ops. Custom `brtarget22` and `imm16op` operand types with encoder methods. `GPRz` RegisterOperand with `GIZeroRegister=R0` for automatic zero-register substitution. Pseudos: RET, LEAfi, SELECT_GPR, SELECT_CC_GPR, ADJCALLSTACK{DOWN,UP} |
