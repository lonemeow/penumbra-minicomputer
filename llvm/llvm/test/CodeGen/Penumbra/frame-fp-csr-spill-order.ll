...
;   1. sub r14, frame_size       — allocate frame
;   2. stw r10, [r14 + N]        — save caller's R10 to CSR slot, SP-relative
;   3. (other CSR spills, all SP-relative)
;   4. mov r10, r14              — set up new FP
;
; And the symmetric required epilogue order:
;   1. mov r14, r10              — restore SP from FP
;   2. ldw r10, [r14 + N]        — restore caller's R10 from CSR slot, SP-relative
;   3. (other CSR restores, SP-relative)
;   4. add r14, frame_size
...
; The R10 spill MUST come before `mov r10, r14`.  The CSR slot MUST be
; addressed as [r14 + ...] (SP), not [r10 + ...] (FP) — at the time of
; the spill R10 still holds the caller's value, not the new FP.
;
; CHECK:         sub r14, {{[0-9]+}}
; CHECK:         stw r10, [r14 + {{[0-9]+}}]
; CHECK-NOT:     mov r10, r14
; CHECK:         mov r10, r14
;
