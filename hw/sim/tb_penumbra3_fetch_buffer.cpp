// Verilator testbench for penumbra3_fetch_buffer.
//
...
// directed corners are named regressions on top of it. The payload is the
// module-default width (133 bits, a VlWide of 5 words); each tag is expanded
// to a per-word signature so any wrong-slot read or partial corruption shows
// up, not just a wrong low word. The real core sets PAYLOAD_W from the carried
// payload; the FIFO logic is width-agnostic, so this exercises it generically.

#include <cstdio>
#include <cstdint>
#include <deque>
#include "Vpenumbra3_fetch_buffer.h"
...
static void tick(Vpenumbra3_fetch_buffer* dut) {
...
static void set_payload(Vpenumbra3_fetch_buffer* dut, uint32_t tag) {
    for (int i = 0; i < NWORDS; i++) dut->i_enq_data[i] = gen_word(tag, i);
}
static bool head_is(Vpenumbra3_fetch_buffer* dut, uint32_t tag) {
    for (int i = 0; i < NWORDS; i++)
        if (dut->o_deq_data[i] != gen_word(tag, i)) return false;
    return true;
}

static void reset(Vpenumbra3_fetch_buffer* dut) {
...
static void drive(Vpenumbra3_fetch_buffer* dut, bool ev, uint32_t tag,
                  bool dr, bool fl) {
...
int main() {
    Vpenumbra3_fetch_buffer* dut = new Vpenumbra3_fetch_buffer;
    reset(dut);
    ...
    printf("penumbra3_fetch_buffer: %d/%d checks passed\n", tests - errors, tests);
