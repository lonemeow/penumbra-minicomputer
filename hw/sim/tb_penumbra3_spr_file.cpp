// Verilator testbench for penumbra3_spr_file.
//
// Drives the SR/EPC/ESR write sources and the RDSPR read mux across clock
// edges, checking the gen3 save-state contract and the SR bit layout:
//   - reset: SR = {S=1, I=0, NZCV=0}; EPC = ESR = 0
//   - NZCV flag commit updates only SR[3:0], leaving S/I
//   - WRSPR EPC / ESR land and read back via RDSPR; RDSPR of a non-held SPR
//     (USP, SR) reads 0 here (those resolve in the regfile / EX)
...
//
// gen3 has no WRSPR SR (decoder-illegal), so SR state is set up through the
// real gen3 paths -- WRSPR ESR then ERET -- rather than a bulk SR load. SR's
// `--assert` invariants ride along: a failed assertion aborts the sim.

#include <cstdio>
#include <cstdint>
#include "Vpenumbra3_spr_file.h"
#include "verilated.h"
...
static void tick(Vpenumbra3_spr_file* dut) {
    dut->i_clk = 0; dut->eval();
    dut->i_clk = 1; dut->eval();
}

// Quiet baseline: no write source active, RDSPR reading SR (reads 0 here).
static void clear(Vpenumbra3_spr_file* dut) {
    dut->i_flag_we = 0; dut->i_flag_value = 0;
    dut->i_save_state = 0; dut->i_save_pc = 0;
    dut->i_eret = 0; dut->i_ei = 0; dut->i_di = 0;
    dut->i_spr_we = 0; dut->i_spr_sel = 0; dut->i_spr_value = 0;
    dut->i_rd_sel = SPR_SR;
}

// Set an arbitrary SR via the gen3 path: WRSPR ESR then ERET (SR <- ESR). This
// leaves ESR == v as a side effect; callers that then check ESR re-establish it.
static void set_sr(Vpenumbra3_spr_file* dut, uint32_t v) {
    clear(dut);
    dut->i_spr_we = 1; dut->i_spr_sel = SPR_ESR; dut->i_spr_value = v;
    dut->eval(); tick(dut);
    clear(dut);
    dut->i_eret = 1;
    dut->eval(); tick(dut);
...
int main(int argc, char** argv) {
    ...
    Vpenumbra3_spr_file* dut = new Vpenumbra3_spr_file;
    ...
    check("flag_keeps_s", dut->o_sr_s, 1);
    ...
    // RDSPR of an SPR this file does not expose as operand B reads 0.
    dut->i_rd_sel = SPR_USP; dut->eval();
    check("rdspr_usp_zero", dut->o_rd_value, 0);
    dut->i_rd_sel = SPR_SR; dut->eval();
    check("rdspr_sr_zero", dut->o_rd_value, 0);

    // ── Save-state: snapshot pre-entry SR, enter S=1/I=0, keep NZCV ──
    set_sr(dut, 0x40000005);                        // S=0, I=1, NZCV=0x5
    clear(dut);
    ...
    dut->i_eret = 1;
    dut->eval(); tick(dut); clear(dut); dut->eval();
    check("eret_sr",   dut->o_sr_read, 0x40000005);     // == the ESR snapshot
    check("eret_sr_s", dut->o_sr_s, 0);
    check("eret_sr_i", dut->o_sr_i, 1);

    // ── Priority: save-state wins over a coincident flag commit ──
    set_sr(dut, 0x00000003);                        // S=0, I=0, NZCV=0x3
    clear(dut);
    dut->i_save_state = 1; dut->i_save_pc = 0x1000;
    dut->i_flag_we = 1; dut->i_flag_value = 0xC;     // would set NZCV=0xC if it won
    dut->eval(); tick(dut); clear(dut); dut->eval();
    check("prio_entry_nzcv_held", dut->o_sr_flags, 0x3);  // flag commit lost
    check("prio_entry_s",         dut->o_sr_s, 1);

    // ── EI / DI flip SR.I only ───────────────────────────────────
    set_sr(dut, 0x80000000);                        // S=1, I=0, NZCV=0
    clear(dut);
    dut->i_ei = 1;
    dut->eval(); tick(dut); clear(dut); dut->eval();
    check("ei_sets_i",  dut->o_sr_i, 1);
    check("ei_keeps_s", dut->o_sr_s, 1);
