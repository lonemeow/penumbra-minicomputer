// penumbra2_fetch_buffer — elastic decoupling FIFO for the IF2 -> ID seam.
//
// Breaks the combinational stall path from the back end to the fetch enable.
// Without it, ID's this-cycle stall (id_stall, itself the tail of
// i_dmem_busy -> mem_stall -> ex_stall) propagates combinationally all the way
// back to o_fetch_en in IF1 — the core's longest path. Interposing this FIFO
// lets the fetch front end see o_enq_ready (a registered occupancy bit) instead
// of id_stall, so a back-end stall no longer reaches the fetch enable in the
...
// (the core packs the IF2/ID register fields in and unpacks them out), so it
// knows nothing about the pipeline it sits in and can be unit-tested alone.
// DEPTH entries; DEPTH=2 is the minimum that breaks the ready path without
// throttling throughput (a 1-deep register cannot accept on the cycle it
// drains). Deeper also absorbs I-cache-miss fetch bubbles.
//
// Latency: a slot enqueued into an empty FIFO presents at the head the *next*
// cycle (registered storage, no same-cycle bypass) — one extra fetch-to-decode
// stage, paid back many times over by the fmax the broken stall path buys.
//
...
module penumbra2_fetch_buffer #(
    parameter int PAYLOAD_W = 133,
    ...
    always_ff @(posedge i_clk) if (!i_rst) begin
        assert (count <= DEPTH[CNT_W-1:0])
            else $error("penumbra2_fetch_buffer: count exceeded DEPTH");
        assert (!(enq_fire && count == DEPTH[CNT_W-1:0] && !deq_fire))
            else $error("penumbra2_fetch_buffer: enqueue into a full FIFO");
        assert (!(deq_fire && count == '0))
            else $error("penumbra2_fetch_buffer: dequeue from an empty FIFO");
    end
