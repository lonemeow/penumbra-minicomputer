// penumbra2_alu — Penumbra/2 EX-stage compute unit.
//
// Combinational 32-bit ALU: result = f(a, b, op), with ARM-style
// NZCV flag generation. Purely combinational — no clock, no state.
// MUL/DIV are not here; they run on the divmul peer unit and their
// results reach the regfile through the writeback mux.
//
// The twelve single-cycle functions are selected by alu_op
// (penumbra2_pkg, ALU_*). SUB and SBC share the adder by inverting B
// and adding a carry-in (A + ~B + cin = A - B for cin=1). ADC/SBC
// take the carry-in from the SR carry flag; ADD/SUB force it. MOV is
// realised as ALU_PASS (pass operand B; the operand mux puts the
// moved value there).

module penumbra2_alu
    import penumbra2_pkg::*;
(
    input  logic [31:0]           i_a,
    input  logic [31:0]           i_b,
    input  logic [ALU_OP_W-1:0]   i_op,
    ...
    // Carry-in: ADD=0, SUB=1 (completes two's complement), ADC/SBC
    // take the SR carry (ARM: A + ~B + C = A - B - !C for SBC).
    always_comb begin
        case (i_op)
    ...
    // ── Shifts ──────────────────────────────────────────────────
    // B[4:0] is the shift amount; the carry is the last bit shifted
    // out (0 when the shift amount is 0).
    logic [4:0]  shamt;
    ...
    // V — signed overflow. Only the add/subtract family produces a
    // meaningful overflow; every other op clears V.
