...
| L2 passthrough | `rtl/sim/l2_passthrough.sv` | via machine_sim (HAS_L2=1) | Phase-0 L2 cache stub — literal wire-through with the future L2's port shape (`i_cacheable` consumed, ignored). Sits between `cpu_core.o_mem_*` and the shared bus when `HAS_L2=1`; not instantiated when `HAS_L2=0` (default). Lets the bus-shape plumbing land before the real L2's storage logic. See `doc/internals/l2-cache.md`. |
