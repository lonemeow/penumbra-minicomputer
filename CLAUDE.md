...
| CPU core | `rtl/core/cpu_core.sv` | 23 progs | Full CPU: datapath + sequencer + ROM + MMU + cache + fetch + IRQ + MMU traps + BREAK + SYSCALL + privilege traps + illegal instruction trap + WRSYS/RDSYS + RDSPR/WRSPR + BL + sub-word loads/stores. Parameterizable RESET_PC (default 0xFFFF_E000). |
